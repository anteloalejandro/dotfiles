import { exec, execAsync, GObject, monitorFile, property, readFileAsync, register } from 'astal';
...
export default class Brightness extends GObject.Object {
  ...
  set screen(percent: number) {
    ...
    execAsync(`brightnessctl set ${Math.round(percent * 100)}% -d ${screen} -q`).then(() => {
      this.#screen = percent;
      this.notify('screen');
    });
  }
