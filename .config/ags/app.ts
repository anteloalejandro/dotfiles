...
import notification_panel_style from "./widget/notifications/Panel.scss"
...
const show_panel = Variable(false)
const bars: Widget.Window[] = []
...
App.start({
  ...
  main() {
    App.get_monitors().map(
      monitor =>
        bars.push(Bar(monitor, show_popups) as Widget.Window)
    )
  },
})
...
App.start({
  css: notification_panel_style,
  main() {
    App.get_monitors().map(
      (monitor, i) =>
        NotificationPanel(monitor, show_panel, show_popups, bars[i])
    )
  }
})
